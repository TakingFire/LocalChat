...
ws.onmessage = function(data) {
  ...
  el.find('.time').text(time.getHours()%12+':'+(time.getMinutes().toString().length < 2 ? '0' : '')+time.getMinutes());
...
$(document).on('keydown', function(e) {
  if (e.which == 120) { $('.panel').eq(0).toggle() }
  if (e.which == 119) { $('.panel').eq(1).toggle() }
})
